// Keep the vendored upstream implementation byte-for-byte readable while allowing only the two
// naming/loop style lints emitted by the current workspace Clippy version.
