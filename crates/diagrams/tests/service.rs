...
use diagrams::{
    apply_update_to_spec, spec_hash, DiagramPatch, DiagramService, DiagramSpec,
    DiagramUpdateRequest, ExportFormat, NodeStatus,
};
use serde_json::json;
...
#[test]
fn pure_update_applies_hash_bound_patch_without_an_artifact_root() {
    let spec = fixture_spec();
    let expected = spec_hash(&spec).expect("fixture hash");
    let mut patch = empty_patch();
    patch.title = Some("虚构借款争议示意图修订版".to_owned());
    let updated = apply_update_to_spec(spec.clone(), &expected, patch)
        .expect("pure hash-bound update succeeds");
    assert_eq!(updated.title, "虚构借款争议示意图修订版");
    assert_ne!(spec_hash(&updated).expect("updated hash"), expected);

    let stale = apply_update_to_spec(spec, &format!("sha256:{}", "0".repeat(64)), empty_patch())
        .expect_err("stale pure update fails closed");
    assert_eq!(stale.code(), "stale_spec");
}
