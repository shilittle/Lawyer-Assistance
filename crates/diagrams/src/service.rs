...
impl DiagramService {
    ...
    pub fn update(
        ...
        let spec = match (request.artifact_uri.as_deref(), request.base_spec) {
        ...
        let spec = apply_update_to_spec(spec, &request.expected_spec_hash, request.patch)?;
        self.render(&spec)
    }
...
/// Applies a bounded diagram patch without reading or writing the filesystem.
///
/// Approved-workspace integrations use this pure operation before publishing
/// the rendered result as an encrypted work product.
pub fn apply_update_to_spec(
    mut spec: DiagramSpec,
    expected_spec_hash: &str,
    patch: DiagramPatch,
) -> Result<DiagramSpec, DiagramServiceError> {
    ...
    apply_patch(&mut spec, patch, &actual_hash)?;
    Ok(spec)
}

pub fn validation_response(spec: &DiagramSpec) -> DiagramValidationResponse {
