...
mod tests {
    ...
    fn named_lock_serializes_concurrent_threads() {
        ...
        let first = ProviderStoreLock::acquire_named(&name, 1_000).expect("first lock acquires");
    ...
    fn named_lock_reports_bounded_busy_error() {
        ...
        let _first = ProviderStoreLock::acquire_named(&name, 1_000).expect("first lock acquires");
        let contender_name = name.clone();
        let error = thread::spawn(move || {
            ProviderStoreLock::acquire_named(&contender_name, 25).expect_err("contender times out")
        })
        .join()
