...
fn search_sources_fts(
    ...
    let law_like = optional_like(query.law_names.first());
    let article_like = optional_like(query.article_numbers.first());
    let level_like = optional_like(query.effectiveness_levels.first());
    ...
    let mut statement = connection.prepare(
        ...
        WHERE law_articles_fts MATCH :fts_query
          AND (:law_like IS NULL OR documents.title LIKE :law_like ESCAPE '\\')
          AND (:article_like IS NULL OR articles.article_number LIKE :article_like ESCAPE '\\')
          AND (:level_like IS NULL OR documents.effectiveness_level LIKE :level_like ESCAPE '\\')
          AND (
            :case_date IS NULL
            OR (
    ...
    let sources = statement
        .query_map(
            named_params! {
                ":fts_query": fts_query,
                ":law_like": law_like.as_deref(),
                ":article_like": article_like.as_deref(),
                ":level_like": level_like.as_deref(),
                ":case_date": query.case_date.as_deref(),
                ":include_expired": if query.include_expired { 1_i64 } else { 0_i64 },
                ":limit": limit,
            },
            legal_source_from_row,
        )?
...
fn infer_missing_reason(
    ...
    let article_exists: Option<i64> = connection
        .query_row(
            ...
            ",
            (&document_id, &version_id, &article_part),
            |row| row.get(0),
        )
        .optional()?;
...
fn primary_search_text(query: &StructuredLegalQuery) -> Option<String> {
    ...
    if !query.article_numbers.is_empty() {
        return Some(query.article_numbers.join(" "));
    }
...
fn optional_like(value: Option<&String>) -> Option<String> {
    value
