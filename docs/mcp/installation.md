...
Without `--legal-db`, the server uses `LEGAL_DB` then `data/runtime/legal_core.sqlite`. `--config` may still provide `legal_db` for existing launchers. Legacy user-workspace options are ignored by the public profile.

For a v1.0.0 `privacy_workspace` stdio connection, the application backend must already be running on loopback and the client token comes from a file or `MCP_TOKEN`:
