...
本页对应 v1.2.0。详细升级步骤见[升级说明](web/ai-upgrade.md)，已执行的功能、模型、案例检索和限制见[AI 增量验收报告](web/ai-validation.md)。Windows 便携 ZIP、源码与验收资料见 [v1.2.0 发布页](https://github.com/shilittle/Lawyer-Assistance/releases/tag/v1.2.0)，附 SHA-256 校验文件与清单；便携包已完成解压运行和逐文件核验。

## 版本与交付

源码版本为 `1.2.0`，基于 V1.0.0 Web 工作区，使用 Rust server、纯 HTML WebUI 和 MCP。便携包由 `scripts/package_portable.py` 在本地生成：
...
| PDF、图片和 DOCX 内嵌图片 | 通过已配置的云端视觉模型 OCR；TXT/DOCX 正文保留本地提取路径 |
| 脱敏输出 | 统一保存为 UTF-8 纯 TXT，批量操作可导出 ZIP |
| 本地法律检索 | 完整匹配集计数、法律归组/条文平铺、筛选、相关性和日期排序、分页 |
| 历史版本和关联法规 | 默认打开；历史版本正文支持分页读取 |
...
停止旧服务，生成或取得 v1.2.0 便携 ZIP，双击 `Lawyer-Assistance.vbs`。首次打开旧 Web 工作区时，后台会先通过 SQLite Backup API 创建 `workspace.pre-ai-v1.sqlite` 并检查完整性，再事务升级到 `web-workspace-v2`；分组、原件、字典、收藏、结果和会话保留。旧 Tauri 工作区不迁移。
