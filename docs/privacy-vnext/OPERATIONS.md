...
3. Create a standalone session. The UI displays only an opaque server ID matching `srv_[0-9a-f]{32}`; it never displays the DPAPI-protected descriptor, Credential Manager secret, bearer, database path, workspace path, or ticket.
...
5. Start a new clean host task containing opaque IDs only. Require the exact 15-tool catalog: the five public-law tools and the ten approved workspace tools.
