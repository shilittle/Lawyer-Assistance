...
The only substantive sinks are `case_write_work_product` and `case_update_work_product`. Both bind approved source references and use immutable/idempotent versioning. Every create/update must be verified through an immediate exact-version `case_read_work_product`; no write response alone is a trusted final result. No tool accepts a host path or export destination.
