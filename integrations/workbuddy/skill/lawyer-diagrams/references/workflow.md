...
1. 确认数据仅为纯虚构或公开材料。任何真实案件材料，即使 Privacy 已批准，也立即改走 `approved_case_workspace`，不得继续此工作流。
...
6. `diagram.render({"schema_version":1,"spec":{...}})`：为纯虚构/公开数据生成本地明文内容寻址 HTML，保存 URI 和 spec hash。
...
这里的 `artifact_uri`、`base_spec` 和 HTML 导出只属于 synthetic/public raw contract。真实案件不使用 artifact URI：approved profile 通过不透明 work-product ID/version、精确 approved source references、加密存储和 descriptor-only export 工作。
