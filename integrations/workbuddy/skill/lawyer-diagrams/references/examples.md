...
调用时读取整个 JSON 对象作为 `spec`，不得把文件路径或尖括号占位字符串提交给 MCP。所有文件均为纯虚构测试数据，下面含 `artifact_uri` 的调用只适用于 `diagram_authoring` 的 synthetic/public 明文制品契约。
...
真实案件（包括已经 Privacy 批准的案件）不得套用这些 URI 示例。改用 `lawyer-assistance-approved-workspace` Skill、`approved_case_workspace` profile 和独立 `diagram_read`/`diagram_write` grants；该路径返回不透明 work-product ID/version，且不返回 artifact URI、路径或 HTML。
